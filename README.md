...
| `font` | str | `'Helvetica, Arial, Liberation Sans, Nimbus Sans, TeX Gyre Heros, DejaVu Sans'` | Matplotlib font family, or a comma-separated fallback chain tried in order. The default keeps a Helvetica look across platforms, always reaching a metric-compatible grotesque before the last resort: Helvetica (macOS) → Arial (Windows) → Liberation Sans (the Arial clone shipped by almost every Linux distro) → Nimbus Sans / TeX Gyre Heros (libre Helvetica clones) → DejaVu Sans (matplotlib's built-in, always present but visibly different), so a missing family never warns. On a bare Linux box with none of the clones installed it still reaches DejaVu; install e.g. `fonts-liberation` for an Arial/Helvetica look (the Colab demo setup does this automatically). `''` or `'auto'` use matplotlib's own default (`'DejaVu Sans'`); a single name (e.g. `'Arial'`) forces just that one. Serif (matches journal body text): `'Times New Roman'`, `'Georgia'`, `'Palatino'`. To make a hand-built matplotlib figure (one that bypasses `run_save()` entirely) match kbstat's own plots, call the public `Kbstat.apply_font()` before building it, then use `fontweight='bold'` on the labels/ticks that should match |
