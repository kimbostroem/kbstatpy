...
| `interaction` | list / str / int | `'auto'` | Which factors may interact. The default `'auto'` fits every interaction **the design can support**; `''` gives the additive model (main effects only), which is now something you ask for rather than what you get by omission. You can also name the terms — `'A, B'` → `A * B`, or `[['A','B'], ['C','D']]` for separate terms — or give an integer `n` for every factor in `x` up to that order (R's `(A+B+C)^n`), or `'all'` for the full factorial. `'auto'` and `'all'` agree whenever the design is complete; they differ only when a term is unestimable because cells are empty, where `'auto'` leaves it out and says so in `Summary.txt` while `'all'` (and an explicit order or term list) raises, naming the term and the missing cells. Estimability is read off the model matrix alone, never the response, so this is not model selection — see [STATISTICAL_NOTES.md](STATISTICAL_NOTES.md). A *partially* estimable term is kept. Order ≥ the factor count is the full factorial; `1` is additive. `'auto'` and `'all'` are reserved factor names |
