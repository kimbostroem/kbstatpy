...
All list-valued options (`x`, `covariate`, `slope`, `interaction`, `y_units`, `x_units`, `correlation`, `correlation_control`) accept either a Python list or a comma-separated string — whichever is more convenient — and all default to `''`.
...
| `id` | str | `''` | Random-effect grouping variable (e.g. subject ID). Several, comma-separated, are **crossed** — one random intercept each: `'subject, session'` → `(1 | subject) + (1 | session)`. lme4's nesting operators work inside a name and mean what they do in lme4: `'subject/trial'` → `(1 | subject) + (1 | subject:trial)`, `'subject:trial'` → `(1 | subject:trial)`. Use the nested form for a replicate index whose labels recur inside each subject (`1, 2, 3 …` per subject) — read as crossed it would pool replicate 1 across all subjects; kbstatpy inspects the data and warns when it sees that shape, and also when a crossed grouping factor has fewer than three levels. Nesting is not automatically the answer: if the data do not support a block term it fits as a zero variance component and a singular fit, and a single grouping factor is the better model. Random slopes (`slope`) attach to the first grouping factor; the rest get intercepts only |
| `slope` | list / str | `''` | Variables with random slopes (e.g. `'A, B'` → `(1 + A + B \| id)`) |
| `slope_correlated` | bool or str | `'auto'` | Random-effect covariance structure for the slopes: `True` (full covariance `(1 + s \| id)`), `False` (uncorrelated/diagonal — glmmTMB `diag(1 + s \| id)` for non-gaussian families, lme4 `(1 + s \|\| id)` for gaussian LMMs), or `'auto'` (default: fit correlated, then refit diagonal only if that fit is singular). The diagonal structure drops the correlation parameters and escapes the singular fits a many-level factor slope can otherwise produce. The structure actually used is reported in `Summary.txt` and the diagnostics footer. Ignored when an explicit `formula` is given. See [Random slopes in GLMMs](STATISTICAL_NOTES.md#random-slopes-in-glmms-pymer4-bug-and-workaround). The on/off spellings above apply here too, with `'auto'` as the third value |
| `interaction` | list / str / int | `''` | Which factors may interact. Name the terms — `'A, B'` → `A * B`, or `[['A','B'], ['C','D']]` for separate terms — or describe the structure and let kbstatpy write them out: an integer `n` gives every factor in `x` up to that order (R's `(A+B+C)^n`), `'all'` the full factorial, `'auto'` every interaction **the design can support**. `'auto'` and `'all'` agree whenever the design is complete; they differ only when a term is unestimable because cells are empty, where `'auto'` leaves it out and says so in `Summary.txt` while `'all'` (and an explicit order or term list) raises, naming the term and the missing cells. Estimability is read off the model matrix alone, never the response, so this is not model selection — see [STATISTICAL_NOTES.md](STATISTICAL_NOTES.md). A *partially* estimable term is kept. Order ≥ the factor count is the full factorial; `1` is additive. `'auto'` and `'all'` are reserved factor names |
...
| `correlation` | list / str | `''` | Variables for pairwise Pearson correlation (must be numeric). Produces scatter grid, colour table, and Excel output. Also accepted as `correlate` |
| `correlation_method` | str | `'pearson'` | Correlation coefficient for both the raw and the partial tables: `'pearson'` or `'spearman'` (rank-based, robust to outliers and monotone non-linearity) |
| `correlation_control` | list / str | `''` | Variable(s) partialled out of every correlation, e.g. `'Age'`. Adjusts both the raw and the partial tables; the control variables themselves are not shown in the matrix |
| `constraints` | str | `''` | Row filter applied before analysis, e.g. `'Year > 1950'` or `'group != "control"'`. Standard Python operators: `==` `!=` `<` `>` `<=` `>=`; combine with `&` (and) / `\|` (or). Also accepted as `constraint` |
...
| `kr_max_obs` | int | `5000` | Observation count above which `df_method='auto'` uses Satterthwaite instead of Kenward-Roger. A cost threshold, not a statistical one: KR's cost grows as roughly `n^2.5` (80 s on 18 000 rows against a flat tenth of a second for Satterthwaite) while the two methods become interchangeable above about a thousand observations, so the default leaves a wide margin and drops KR only where it gets expensive. An explicit `df_method='kenward-roger'` is still honoured at any size (with a warning); `0` removes the cap. See [STATISTICAL_NOTES.md](STATISTICAL_NOTES.md) for the measured curve |
| `model_comparison` | bool | `False` | Report maximum-likelihood AIC/BIC for a ladder of fixed-effect structures (additive, all two-way, full factorial) beside the fitted model, so the cost of the additive default is visible rather than implicit. The random effects are held fixed across the rungs. **A report, not a selection**: choosing a structure by AIC and then quoting the winner's p-values inflates them badly (over 300 simulated datasets with no interaction present, searching this ladder kept one 30.7% of the time and called it significant in 13.0% of runs, against 5.0% for a pre-specified test). The table is followed by a reading of it: which structure the criteria favour, or that they favour none (structures within 2 AIC are reported as indistinguishable, and parsimony breaks the tie). Written to `ModelComparison.xlsx`. |
...
| `posthoc_family` | str | `'cell'` | Scope of the family `posthoc_correction` is applied over when the comparison is conditional. `'cell'` (default) each cell is its own family, corrected independently — the emmeans default (`pairs(…, by = )`). **With a two-level factor each family holds a single contrast, so the correction is the identity and `pCorr` equals `p`, and the cells are not corrected for one another** — `Summary.txt` now says so outright. `'pooled'` every conditional contrast forms one family, corrected in a single pass; requires a poolable `posthoc_correction` (an R `p.adjust` method). `'cross'` two-stage — corrected within each cell, then Bonferroni across the cells, so each cell is held to α / n_cells and the union is controlled at α; for the exact within-family methods (`'tukey'`, `'mvt'`, `'dunnettx'`), which have no pooled form. With one contrast per cell `'pooled'` is uniformly at least as powerful as `'cross'`, and kbstatpy warns if you pick the dominated one; with two or more, neither dominates. The marginal (`'any'`) block is excluded from every family — it is the ANOVA term for that factor, not an extra test. See [STATISTICAL_NOTES.md](STATISTICAL_NOTES.md) |
...
| `show_group_size` | bool | `False` | Annotate each plotted group with its observation count (`n=12`), placed just above the group's violin (or its CI bar in bar style). Off by default in both styles — note that before this option existed the counts were drawn unconditionally in bar style, so bar plots no longer show them unless you ask. The significance brackets are stacked above the labels and keep a visible gap from them, so switching the labels on pushes the brackets up instead of colliding with them. `'true'`/`'false'`, `'on'`/`'off'`, `'yes'`/`'no'` and `'none'` (= off) are accepted alongside `True`/`False` |
| `show_emm_lines` | bool or str | `False` | Draw a horizontal reference line across the whole panel at the height of every plotted group's EMM (the marginal mean already marked by the white dot), in that group's own colour, so a group's level can be read off directly against the other groups' distributions instead of comparing dot heights by eye across the panel. `False` (default) no lines; `True` lines in the default style, **dotted**, which recedes furthest behind the violins and the significance brackets so a line crossing a violin cannot be mistaken for plotted data; or give a line style — `'-'`, `'--'`, `':'`, `'-.'`, or the matplotlib names `'solid'`, `'dashed'`, `'dotted'`, `'dashdot'` — where solid reads calmest and makes the group colours easiest to attribute, at the cost of looking more like plotted content. Applies to the data plots in both violin and bar style; each panel uses its own EMMs, and where no EMM is available the line follows the same median fallback as the dot. The Matlab-style strings `'true'`/`'false'` are accepted too, and `'none'`/`'off'` mean off, so ported scripts work unchanged |
...
| `data_outliers` | str | `'text'` | How data outliers (flagged by `remove_outliers_prefit`/`remove_outliers_postfit`) appear in the data plot. `'text'` (default) omits the points but annotates the count and percentage at the bottom of each panel, so the y-axis autoscales to the non-outlier data — useful when extreme outliers otherwise squash the plot; `'plot'` draws each outlier as a red X marker; `'none'` omits them entirely (`'off'` / `'hide'` accepted). (Renamed from `show_outliers`, which still works with a deprecation warning.) |
| `diagnostic_sims` | int/str | `'auto'` | Number of simulated datasets behind the DHARMa quantile residuals in the diagnostic distribution panels. This sets their *resolution*: a quantile residual can only take the values `k/n_sim`, so the most extreme value expressible is `qnorm(1/n_sim)` and everything past it is capped. DHARMa's own default of 250 puts that at z = ±2.65, which shows up as horizontal rows of points at the ends of the Q-Q plot and inflates the capped count (one real fit: 1.44% capped at 250 simulations, 0.20% at 3028). `'auto'` asks for 2 × n_obs, bounded to [1000, 5000] and then held under a memory budget, since DHARMa keeps an n_obs × n_sim matrix. Give an integer to pin it. |
