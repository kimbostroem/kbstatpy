...
| `profile_across` | str | `''` | Level-wise profile analysis across one ordered categorical factor B (must be in `x`). In addition to the normal analyses, profiles how the factor(s) that interact with B behave across B's ordered levels: each interacting factor's pairwise contrast *within* every level of B (Layer 1), and the interaction as a focused 1-df **linear trend** across B's positions reported next to the factor-omnibus (Layer 2). Level order from `x_order[B]` else B's existing order; the linear trend is equal-spaced (polynomial) along that order, while the plot x-axis uses the labels' numeric values when all parse (else ranks). Writes `LevelProfile.xlsx` + a profile plot. Meaningful when B interacts with the profiled factor and has ≥3 ordered levels (warns otherwise). See [Level-wise profile analysis](#level-wise-profile-analysis) |
...
Level order is taken from `x_order[B]` if set, else B's existing order. The linear
trend uses **equal-spaced** polynomial positions along that order (so for now,
genuinely unequal numeric spacing is treated as equally spaced in the *trend test*,
though the profile plot's x-axis still uses the labels' true numeric values when
they all parse as numbers). The analysis is meaningful only when B
