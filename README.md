...
**Statistical properties of each demo** — see [STATISTICAL_NOTES.md](STATISTICAL_NOTES.md) for full discussion:

- **Demo 1** — identical to an independent-samples t-test (F = t², same df and p-value; balanced data)
- **Demo 2** — equivalent to a paired t-test for balanced complete data; Satterthwaite df ≈ n − 1, slightly more conservative, generalises to missing data
- **Demo 3** — identical to a classical two-way factorial ANOVA with Type III SS (10 observations per cell; balanced)
...
- **Demo 6 vs. 7** — log-transform LMM and gamma GLMM are both valid for right-skewed positive data; gamma is preferred when variance scales with the mean (multiplicative noise)
- **Demo 8** — partial interactions keep the model parsimonious; classical ANOVA always tests all pairwise interactions
- **Demo 9** — multiple dependent variables multiply family-wise error; within-model post-hoc is Holm-corrected, but cross-variable correction is the researcher's responsibility
- **Demo 10** — partial correlation removes shared variance from third variables; Pearson r alone can be misleading when multiple variables co-trend
- **Demo 11** — VIF flags collinearity among numeric predictors; see the VIF section in STATISTICAL_NOTES
