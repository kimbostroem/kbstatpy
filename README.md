...
| `font` | str | `''` | Matplotlib font family for all plots. Empty string uses matplotlib's default (`'DejaVu Sans'`). Common choices: `'Arial'`, `'Helvetica'`, `'Times New Roman'`, `'Georgia'`, `'Palatino'`, `'Verdana'`, `'Calibri'`, `'Roboto'`, `'Liberation Sans'` (Arial substitute, always available on Linux). Sans-serif fonts (`'Arial'`, `'Helvetica'`) are standard for scientific figures; serif fonts (`'Times New Roman'`, `'Georgia'`) match journal body text |
