...
The fitted trend line is drawn **only where the pattern is close to collinear**, specifically where the largest deviation of the level estimates from the fitted line is under half the slope magnitude. A 1-df linear contrast can reach significance on a rise-then-fall pattern, since it weights the endpoints; drawing a straight line through that would assert a monotone gradient the data do not show. In that case the p-value is annotated and no line is drawn, so the figure cannot overstate the result.
