...
When three or more variables are correlated simultaneously, kbstatpy additionally computes **partial correlations**. For each pair (X, Y), both members are residualised on the *same* conditioning set — the remaining correlation variables plus any control variables, **excluding X and Y themselves** — and the Pearson r of those two residual vectors is the partial correlation. This isolates the direct association between X and Y that cannot be explained by the remaining variables, and is particularly valuable when variables co-trend over time or share a common driver (as in the Longley macroeconomic data used in Demo 5).

> **Why the conditioning set must exclude both members.** Residualising X on *all* the other variables (thereby leaving Y in the predictor set) and likewise Y on all the others is a different quantity: the correlation of those two residual vectors is identically **minus** the partial correlation, via the precision-matrix identity `partial_r(i,j) = −P_ij / √(P_ii · P_jj)` with `P = inv(cov)`. Versions up to 1.11.3 did exactly that, so every reported partial coefficient and every partial scatter slope carried the wrong sign while its magnitude was correct. Fixed in 1.11.4 and pinned by `tests/test_partial_correlation_sign.py`, which checks the coefficients against the precision-matrix definition.

**Spearman option.** By default both the raw and the partial correlations are Pearson's. Setting `options.correlation_method = 'spearman'` switches them to **Spearman's rho** — rank correlations that capture any monotone (not only linear) association and are robust to outliers and skew. The Spearman partial correlation is the same residual-of-residuals construction performed on the **ranks**: each variable is rank-transformed, then residualised on the pair's conditioning set. The figure titles name the method.
...
**Two figures, because the trend is a contrast.** `LevelProfile` draws the absolute EMMs, one line per level of A across B. That shows the levels but not the tested quantity: the Layer-2 trend is a trend of the *contrast between* A's levels, and on a link scale at that. Where the levels differ greatly in magnitude the absolute plot is dominated by that magnitude and the contrast gradient is hard to see at all. `LevelProfileContrast` therefore plots the contrast directly, with 95% CIs from the same `emmeans` link-scale estimates the trend is computed from: as a **ratio on a logarithmic axis** (reference line at 1) for a log link, where a constant ratio is a constant distance, and as a plain **difference on a linear axis** (reference line at 0) otherwise.

The fitted trend line is drawn **for every significant trend**, because the p-value is a statement about the slope, so the slope is what the annotation refers to and what should be drawn. The estimates and their intervals are plotted regardless, so a pattern that departs from a straight line appears as points sitting off the line: the line cannot conceal a bend, it simply does not trace one. This is the ordinary logic of a regression plot, where fit and data are shown together and the reader judges the adequacy of the fit.

Two alternatives were tried and rejected. Suppressing the line where the estimates were far from collinear hid real results, since a significant trend then got no line at all. Joining the estimates instead of fitting shows the observed shape but not the tested quantity, and once the profiled factor has more than three levels it degenerates into a zigzag that nothing in the model refers to. Note also that no *departure*-from-linearity statistic is reported: with k levels the departure carries k−2 df, so singling out the quadratic term would be arbitrary for k > 3, and the diffuse alternative is already covered by the factor omnibus reported alongside.
...
`options.x = 'group, eyes, limb'` fits every interaction among those factors that the design can support, because `options.interaction` defaults to `'auto'`. The alternative, `interaction = ''`, fits an **additive** model, and that is emphatically not a neutral choice: it asserts that each factor's effect is the same at every level of the others, and the post-hoc contrasts then come out identical in every cell because the model forbids them from differing. `Summary.txt` names the structure under MODEL INFORMATION, and the post-hoc table blanks the repeated values and says why.

Additive was the default up to version 1.21.0, and the reason it is not any more is worth stating, because it is the same reason the caution above exists. On a three-factor dataset from a real study, the additive fit reported the group difference as `p = 1.5 × 10⁻⁸` and identical in all four cells of limb × eyes. Fitting the interactions the design supports showed a significant `group:eyes` interaction (`p = .0095`) and four cells that differ substantially, one of them with no group difference at all (`p = .20`). The additive model had not merely lost power; it had reported a uniform effect that the data contradict. An untested assumption of no interaction is still an assumption, and making it silently is worse than testing it.

What `'auto'` does *not* do is choose between structures by fit. It fits every interaction the design can estimate and nothing more, which is a decision about the design, not about the response — see [Describing the structure instead of listing the terms](#describing-the-structure-instead-of-listing-the-terms). Which structure is best supported by the data is a separate question, and `options.model_comparison` will report maximum-likelihood AIC and BIC for a ladder of them (additive, all two-way, full factorial), with the random effects held fixed so only the fixed effects vary.

The cost of the richer default is degrees of freedom, and it grows quickly with the number of factors (see the table below). Where an interaction is genuinely implausible or the sample is too small to carry it, say so explicitly with `interaction = ''` or an order limit such as `interaction = 2`, and `Summary.txt` will record the structure you chose.
...
There is one place the `df = Inf` cannot be used directly: the **standardized effect sizes**. The pairwise SMD (Cohen's d) and the partial η² are derived from the contrast's `F = t²` via `d = 2·√(F/df)` and `η² = F/(F + df)`, and both need a *finite* df — at `df = Inf` they collapse (`d → 0`, `η²` undefined), which is uninformative. kbstatpy therefore computes the asymptotic post-hoc effect sizes with the residual df `n − p` in the denominator, exactly the `fitglme` convention above, so they are non-zero and comparable to the preceding MATLAB analyses (the Gaussian LMMs keep their finite Kenward-Roger / Satterthwaite df). This affects only the descriptive effect-size columns; the tests themselves still use the principled asymptotic `df = Inf`. Because `n − p` treats the repeated within-subject observations as independent it overstates the degrees of freedom, so these effect sizes are somewhat **liberal** (approximate upper bounds) and should be read as rough magnitudes. `Summary.txt` flags this beside the post-hoc table.

The **verbal effect-size labels** (`very small` … `very large`) use a single seven-bin scheme for partial η², Cohen's d, and the correlation coefficients (Pearson r and Spearman rho), reproducing the MATLAB kbstat `effprint` bins: the three Cohen anchors (η² 0.01/0.06/0.14, d 0.2/0.5/0.8, r and rho 0.1/0.3/0.5) plus the midpoint "X to Y" categories between them. Since d is a monotone function of η², labelling either metric gives essentially the same word, so the ANOVA table (labelled from η²) and the post-hoc table (also labelled from the contrast's η²) stay consistent.
...
**When kbstatpy computes VIF:** for every fit with two or more numeric covariates. That means `options.covariate` in practice, since `options.x` is cast to factors before fitting and a factor's contrasts have no single variance to inflate — a numeric variable placed in `x` becomes a grouping factor with one level per distinct value, not a continuous predictor. `options.scale_covariates`, on by default, centres and scales those covariates. It changes nothing about the VIF, which is scale-invariant, and nothing about any test: a covariate that is not in an interaction has its coefficient and its standard error divided by the same number, and estimated marginal means are evaluated at the covariate means either way. What it changes is the conditioning of the optimisation, which matters only where an ill-scaled model would otherwise struggle to converge — it does not improve the fit in any statistical sense, the likelihood being identical. `Data.csv` keeps each covariate in its own units and adds the fitted z-scores beside it as `<name>_scaled`. Up to version 1.23.2 the calculation sat inside the correlation analysis and so ran only when `options.correlation` was set, which meant a model with severely collinear covariates could be reported without a word about it.
...
The number of simulations sets the **resolution** of these residuals, and is worth understanding before reading their tails. A quantile residual is the observation's position in the empirical CDF of the simulated datasets, so it can only take the values `k/n_sim`: the most extreme value expressible is `qnorm(1/n_sim)`, and every observation beyond it lands either on that value or in the capped pile. At DHARMa's own default of 250 simulations that limit is z = ±2.65, which on a few thousand rows appears as **horizontal rows of points at the ends of the Q-Q plot** — the grid running out, not ties in the data. It also inflates the capped count, which is therefore not a property of the fit alone: one real gamma GLMM on 6603 observations reported 1.44% capped at 250 simulations and 0.20% at 3028, same model, same data. Two things follow from that discreteness. First, kbstatpy draws the residual from **within** its grid interval rather than snapping it to the edge: under the null an observation lying between simulation order statistics *k* and *k+1* has a true quantile uniform on `(k/n, (k+1)/n)`, so the randomised (Dunn–Smyth) value is the exact one and the rung edge is the approximation. The randomised residual is exactly Uniform(0, 1), hence exactly N(0, 1), under a correct model, where the snapped one is only approximately so. It is seeded, so figures reproduce. Second, the simulation count still governs how far the tails can reach, so it is worth setting well.

`options.diagnostic_sims` controls this; `'auto'` (the default) asks for 2 × n_obs, the resolution needed to express the most extreme order statistic of an n_obs sample, bounded to [1000, 5000] and then held under a memory budget, since DHARMa retains an n_obs × n_sim matrix of simulated responses. `Summary.txt` reports the count used and the resolution limit it implies.

Because the quantile residuals cap any out-of-envelope observation at z = ±7, such points pile up as an edge spike in the histogram and a horizontal band at the extremes of the Q-Q. These are DHARMa's **outliers** — observations more extreme than every simulated dataset, a model-vs-data misfit / heavy-tail signal, and a distinct concept from the interquartile-range data outliers removed before fitting (see `data_outliers`). These points are **never drawn**: ±7 is where DHARMa parks anything off the simulated scale, so a marker there would invent a value that does not exist, and it would compress the rest of the panel into a fraction of its axis. They are dropped from the histogram and the Q-Q, and `Summary.txt` records how many there were.

The count is bookkeeping about the plot, not a diagnostic, and it is deliberately not stamped on the figure. It barely separates a sound model from a broken one — on one real dataset the model actually used gave 9× the null rate of `2/(n_sim+1)` against 20× for the wrong distribution family, while the full residual distribution separated the same two by seventy orders of magnitude in a KS test. It also moves with the simulation count and with which rows are in the model frame (including 497 already-flagged outlier rows tripled it). Whatever it has to say about tail misfit, the Q-Q beside it says better; what it uniquely records is that some observations are missing from those two panels, and that belongs in the text.

One thing worth being explicit about: **the simulations serve the diagnostic panels only.** The model fit, the ANOVA, the post-hoc contrasts and the estimated marginal means are all computed from the observed data; no simulated value enters any reported estimate, statistic or p-value. Either way the Normal reference curve on the histogram is computed from the non-capped residuals, so the ±7 pile never inflates its spread. A non-trivial fraction of capped points is worth heeding: DHARMa's own `testOutliers()` formally tests whether there are more of them than the simulation rate would predict.
...
**Correlated vs uncorrelated slopes (`slope_correlated`).** The *correlated* random-effect structure `(1 + s | id)` estimates the full covariance among the random intercept and every random slope — the intercept-slope and slope-slope correlations included. For a slope on a categorical factor with *k* levels this covariance has *k(k+1)/2* parameters, and with modest per-subject data the data rarely pin all of them down. When they cannot, the estimated random-effect correlation matrix collapses toward a rank-deficient (all-±1) form: glmmTMB then reports a **non-positive-definite Hessian** and returns a **NaN marginal likelihood** (so AIC/BIC/logLik are NaN), while lme4 refuses the fit outright once the random effects outnumber the observations. In the glmmTMB case the fixed-effect Wald tests and the EMMs from the same fit remain valid even though the likelihood is NaN. The *uncorrelated* (diagonal) structure keeps a separate random variance for the intercept and for each slope but forces their correlations to zero, dropping the off-diagonal parameters that cause the singularity — glmmTMB `diag(1 + s | id)` for the non-gaussian families, lme4's `(1 + s || id)` for gaussian LMMs. It still lets each subject have their own per-level deviation (the scientific point of the random slope), converges cleanly, and typically wins on AIC over an intercept-only model, so it is the right reduction when the correlated slope is singular rather than dropping the random slope altogether. Dropping the intercept, `(0 + s | id)`, does *not* help: it is the same correlated covariance re-parameterised, so it stays singular — the fix is the diagonal structure, not the intercept.

The `slope_correlated` option chooses between these. The default, **`'auto'`**, fits the correlated structure first and refits with the diagonal one *only* when the correlated fit is singular (non-positive-definite Hessian, boundary correlation, non-finite likelihood, or an outright lme4 fit failure). This keeps the richer, R-conventional model whenever the data support it and falls back to the diagonal structure only when they don't, so you get a usable fit without having to pre-diagnose the singularity. **`True`** always keeps the correlated structure and, if it comes back singular, warns and points you at `'auto'`/`False` rather than silently returning a NaN-likelihood fit. **`False`** always fits the diagonal structure. Whichever structure is used is stated in `Summary.txt` (the "Random-slope structure" line) and, when it is not the plain correlated default, in the diagnostics-plot footer; an `'auto'` fallback is flagged as auto-selected. The choice is ignored when an explicit `formula` is supplied, since that string already specifies the random-effect term verbatim.

One caveat for gaussian LMMs: lme4's `||` decorrelates the intercept from the slope and decorrelates *distinct* slope terms, but it does **not** decorrelate the individual levels *within* a single categorical slope term (it expands to `(1 | id) + (0 + factor | id)`, and that second block is still a correlated covariance over the factor's levels). glmmTMB's `diag()` decorrelates fully, including within a categorical slope, so the diagonal benefit for a many-level factor slope is realised cleanly for the non-gaussian families; for a gaussian LMM with a categorical random slope, achieving a fully diagonal structure requires expanding the factor into indicator terms (e.g. `afex::mixed(expand_re = TRUE)`), which kbstatpy does not do automatically. This is a narrow corner (numeric slopes are already decorrelated by `||`, and the non-gaussian families are handled by `diag()`), so it is left as possible future work: either the indicator expansion, or a small option to fit a gaussian model through glmmTMB — which would gain the correct `diag()` at the cost of lme4's Kenward-Roger / Satterthwaite denominator df. Until then, if a gaussian model genuinely needs a diagonal categorical slope, refit it as a glmmTMB family (accepting the asymptotic Wald df).
