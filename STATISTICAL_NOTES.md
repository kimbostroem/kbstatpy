...
The **verbal effect-size labels** (`very small` … `very large`) use a single seven-bin scheme for both partial η² and Cohen's d, reproducing the MATLAB kbstat `effprint` bins: the three Cohen anchors (η² 0.01/0.06/0.14, d 0.2/0.5/0.8) plus the midpoint "X to Y" categories between them. Since d is a monotone function of η², labelling either metric gives essentially the same word, so the ANOVA table (labelled from η²) and the post-hoc table (also labelled from the contrast's η²) stay consistent.
