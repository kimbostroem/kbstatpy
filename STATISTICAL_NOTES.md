...
**Correlated vs uncorrelated slopes (`slope_correlated`).** By default kbstatpy fits the *correlated* random-effect structure `(1 + s | id)`, which estimates the full covariance among the random intercept and every random slope — the intercept-slope and slope-slope correlations included. For a slope on a categorical factor with *k* levels this covariance has *k(k+1)/2* parameters, and with modest per-subject data the data rarely pin all of them down. When they cannot, the estimated random-effect correlation matrix collapses toward a rank-deficient (all-±1) form: glmmTMB then reports a **non-positive-definite Hessian** and returns a **NaN marginal likelihood** (so AIC/BIC/logLik are NaN), even though the fixed-effect Wald tests and the EMMs from the same fit remain valid. Setting `slope_correlated = False` fits the *uncorrelated* (diagonal) structure instead — glmmTMB `diag(1 + s | id)` for the non-gaussian families, lme4's `(1 + s || id)` for gaussian LMMs. This keeps a separate random variance for the intercept and for each slope but forces their correlations to zero, dropping the off-diagonal parameters that were causing the singularity. The diagonal fit still lets each subject have their own per-level deviation (the scientific point of the random slope), converges cleanly, and typically wins on AIC over an intercept-only model, so it is the right reduction when the correlated slope is singular rather than dropping the random slope altogether. Note that dropping the intercept, `(0 + s | id)`, does *not* help: it is the same correlated covariance re-parameterised, so it stays singular — the fix is the diagonal structure, not the intercept.
